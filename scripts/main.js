...
document.querySelector('.js-view-favs-btn').addEventListener('click', showFavs);
...
function favBtnUpdate(){
  var favArray = photoArray.filter(function(photoInst){
...
var allCards = document.querySelectorAll('.js-photo-card');
  ...
  allCards.forEach(function(card) {
    var svg = card.children[3].children[1].src;
    if (!svg.includes('active')) {
       card.classList.add('hidden');
    } else {
      card.classList.remove('hidden');
    } 
